...
export async function loadSplat(model) {
    let bytesRead = 0;
    try {
    ...
    async function parseSplatAndSetSplatData(model, perByteLen, perValue, newValue) {
        return new Promise(async (resolve) => {
            ...
            const fnParseSplat = async () => {
                if (cntSplat > maxProcessCnt) {
                    const data = await parseSplatToTexdata(value, maxProcessCnt);
                    ...
                    const data = await parseSplatToTexdata(value, cntSplat);
                    setSplatData(model, data);
