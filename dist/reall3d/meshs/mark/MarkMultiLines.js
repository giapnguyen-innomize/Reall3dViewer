...
export class MarkMultiLines extends Line2 {
    ...
    drawStart(startPoint, name) {
        if (this.disposed)
            return;
        const cnt = document.querySelectorAll('.mark-wrap-lines.main-warp').length + 1;
        const data = {
            type: 'MarkMultiLines',
    ...
    drawUpdate(data, saveData = true, lastPoint, next = false) {
        ...
        if (lastPoint) {
            if (next) {
                ...
                that.css3dTags[index].element.childNodes[0].innerText = title;
    ...
    updateByMeterScale(meterScale) {
        ...
        for (let i = 1; i < points.length; i++) {
            ...
            that.css3dTags[i - 1].element.childNodes[0].innerText = (start.distanceTo(end) * meterScale).toFixed(2) + ' m';
        }
    }
    /**
     * 绘制结束
