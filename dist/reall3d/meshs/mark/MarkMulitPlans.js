...
import { AddMarkToWeakRef, ComputePlansArea, ComputePlansCenter, DeleteMarkWeakRef, GetOptions, GetScene, MarkFinish, ReComputePlansArea, StopAutoRotate, TraverseDisposeAndClear, ViewerNeedUpdate, } from '../../events/EventConstants';
export class MarkMultiPlans extends Line2 {
    ...
    drawStart(startPoint, name) {
        if (this.disposed)
            return;
        const that = this;
        const cnt = document.querySelectorAll('.mark-wrap-plans.main-warp').length + 1;
        const data = {
            type: 'MarkMultiPlans',
     ...
     * 绘制更新
     */
    drawUpdate(data, saveData = true, lastPoint, next = false) {
        if (this.disposed)
            return;
        const that = this;
        if (lastPoint) {
            if (next) {
                ...
                const index = that.css3dTags.length - 1;
                const start = new Vector3().fromArray(that.data.points.slice(len - 6, len - 3));
                const end = lastPoint;
                ...
                const index = that.css3dTags.length - 1;
                const start = new Vector3().fromArray(that.data.points.slice(len - 6, len - 3));
                const end = new Vector3().fromArray(that.data.points.slice(len - 3));
                const midPoint = new Vector3((start.x + end.x) / 2, (start.y + end.y) / 2, (start.z + end.z) / 2);
                const distance = start.distanceTo(end);
                const title = (distance * that.events.fire(GetOptions).meterScale).toFixed(2) + ' m';
                that.css3dTags[index].element.innerText = title;
                ...
                const center = that.events.fire(ComputePlansCenter, that.data.points);
                that.css3dAreaTag.position.copy(center);
                const area = that.events.fire(ComputePlansArea, that.data.points);
                that.css3dAreaTag.element.childNodes[0].innerText = area.toFixed(2) + ' m²';
            }
        }
        if (data?.lineColor) {
            saveData && (that.data.lineColor = data.lineColor);
            that.material.color.set(data.lineColor);
            that.meshPlans.material.color.set(data.lineColor);
        }
        if (data?.lineWidth) {
        ...
        if (data?.areaTagColor) {
            ...
            const tag = document.querySelector(`.${that.data.name}-area-tag`);
        ...
        if (data?.distanceTagVisible !== undefined) {
            ...
            that.css3dTags.forEach((tag) => (tag.visible = data.distanceTagVisible));
        }
        if (data?.planOpacity) {
            saveData && (that.data.planOpacity = data.planOpacity);
            this.meshPlans.material.opacity = data.planOpacity;
        }
    ...
    updateByMeterScale(meterScale) {
        ...
        for (let i = 1; i < points.length; i++) {
            ...
            that.css3dTags[i - 1].element.innerText = (start.distanceTo(end) * meterScale).toFixed(2) + ' m';
        }
        const area = that.events.fire(ReComputePlansArea, points, meterScale);
        that.css3dAreaTag.element.childNodes[0].innerText = area.toFixed(2) + ' m²';
    }
    /**
     * 绘制结束
     */
    drawFinish(hasSelectPoint) {
        if (this.disposed)
            return;
        const that = this;
        const len = that.data.points.length;
    ...
    draw(inputData, finish = false) {
        ...
        const count = data.points.length / 3 - 1;
        ...
        const center = that.events.fire(ComputePlansCenter, data.points);
        if (!that.css3dAreaTag) {
            const areaTagWarp = document.createElement('div');
            ...
            that.css3dAreaTag.position.copy(center);
            that.css3dAreaTag.element.childNodes[0].innerText = area;
        }
        that.css3dAreaTag.visible = data.points.length > 6 && data.areaTagVisible;
        for (let i = that.css3dTags.length; i < count; i++) {
            const start = new Vector3().fromArray(data.points.slice(i * 3, i * 3 + 3));
            const end = new Vector3().fromArray(data.points.slice(i * 3 + 3, i * 3 + 6));
            const midPoint = new Vector3((start.x + end.x) / 2, (start.y + end.y) / 2, (start.z + end.z) / 2);
            const distance = start.distanceTo(end);
            const title = (distance * that.events.fire(GetOptions).meterScale).toFixed(2) + ' m';
    ...
    drawPlans(data) {
        ...
        if (data.points.length > 6) {
            ...
            const start = new Vector3().fromArray(data.points.slice(0, 3));
            const end = new Vector3().fromArray(data.points.slice(-3));
