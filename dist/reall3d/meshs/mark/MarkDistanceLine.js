...
export class MarkDistanceLine extends Line2 {
    ...
    drawStart(startPoint, name) {
        if (this.disposed)
            return;
        const that = this;
        const cnt = document.querySelectorAll('.mark-wrap-line.main-warp').length + 1;
        const data = {
    ...
    drawUpdate(data, saveData = true) {
        ...
        if (data?.endPoint) {
            ...
            const end = new Vector3().fromArray(data.endPoint);
            ...
            that.css3dTag.element.childNodes[0].innerText = title;
        }
        if (data?.lineColor) {
        ...
        if (data?.distanceTagColor) {
            ...
            document.querySelector(`.${that.data.name}-distance-tag0`).style.color = data.distanceTagColor;
        }
        if (data?.distanceTagBackground) {
            saveData && (that.data.distanceTagBackground = data.distanceTagBackground);
            document.querySelector(`.${that.data.name}-distance-tag0`).style.background = data.distanceTagBackground;
        }
        if (data?.distanceTagOpacity) {
            saveData && (that.data.distanceTagOpacity = data.distanceTagOpacity);
            document.querySelector(`.${that.data.name}-distance-tag0`).style.opacity = data.distanceTagOpacity.toString();
        }
    ...
    updateByMeterScale(meterScale) {
        const that = this;
        const start = new Vector3().fromArray(that.data.startPoint);
        const end = new Vector3().fromArray(that.data.endPoint);
        const distance = (start.distanceTo(end) * meterScale).toFixed(2) + ' m';
        that.css3dTag.element.childNodes[0].innerText = distance;
    }
    /**
     * 绘制结束
     */
    ...
    draw(inputData) {
        ...
        const start = new Vector3().fromArray(data.startPoint);
        const end = new Vector3().fromArray(data.endPoint);
        const midPoint = new Vector3((start.x + end.x) / 2, (start.y + end.y) / 2, (start.z + end.z) / 2);
        const geometry = new LineGeometry();
        geometry.setPositions([...data.startPoint, ...data.endPoint]);
